...
def all_modules(states, a, c, module):
    buf_states = []
    all = []
    div = divisors(module)
    variants = iter(div)
    for j in range(len(div)):
        example = next(variants)
        l = LCG(states[0], a, c, example)
        for i in range(len(states) - 1):
            buf_states.append(l.next())
        if buf_states == states[1:]:
            all.append(example)
        buf_states.clear()
    return all
...
increment = args.inc
if modulus is None:
    if len(known_elements) < 5:
        print('At least 5 known values are needed to calculate the modulus')
        exit()
...
all_variants = all_modules(known_elements, multiplier, increment, modulus)
print('''Modulus: {}
Multiplier: {}
Increment: {}'''.format(all_variants, multiplier, increment))

if args.next is not None:
    print('\nCalculating next values:')
    for i in range(len(all_variants)):
        lcg = LCG(known_elements[-1], multiplier, increment, all_variants[i])
